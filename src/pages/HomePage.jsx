    ...
    <div>
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-8 py-16 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-6">
          {categories.map((c) => (
            <button key={c.id} onClick={() => navigate(`/jobs?category=${encodeURIComponent(c.name)}`)} className="card rounded-full px-4 py-2 text-sm font-medium hover:shadow-md transition">
              {c.name}
            </button>
          ))}
        </div>
