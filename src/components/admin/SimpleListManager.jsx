    ...
    <div>
      ...
      <div className="card rounded-2xl p-5 mb-5">
        <div className="flex flex-col sm:flex-row gap-2">
          <input value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm w-full" />
          <button
            onClick={() => { if (value.trim()) { onAdd(value.trim()); setValue(""); } }}
            className="btn-primary rounded-lg px-4 py-2 text-sm font-bold w-full sm:w-auto"
          >
            إضافة
          </button>
        </div>
