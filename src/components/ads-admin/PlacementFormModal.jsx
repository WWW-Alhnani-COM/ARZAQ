...
import { AD_PAGES, AD_POSITIONS, AD_PAGES_LABELS, AD_POSITIONS_LABELS } from "../../utils/constants";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-backdrop p-4" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-2xl p-6 fade-up" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">الإعلان</label>
            <select 
              value={form.adId} 
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">الصفحة</label>
            <select 
              value={form.page} 
            ...
            >
              {AD_PAGES.map((p) => (
                <option key={p} value={p}>{AD_PAGES_LABELS[p]}</option>
              ))}
            </select>
          </div>
          
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">الموضع</label>
            <select 
              value={form.position} 
            ...
            >
              {AD_POSITIONS.map((p) => (
                <option key={p} value={p}>{AD_POSITIONS_LABELS[p]}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="submit" className="btn-primary rounded-xl px-6 py-2.5 font-bold text-sm">حفظ</button>
